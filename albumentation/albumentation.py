...
# 이미지 경로 
dataset_directory = os.path.join('./SFR-03(Abnormal)', 'Sagging_Image')
...
def write_image(dataset):
    ...
    for i in range(len(dataset)):
        ...
        if i % 3 == 0:
            cv2.imwrite(f'./SFR-03(Abnormal)/Albumentation_dataset/HARD_{(i//3)+87}.jpg', albumentation_image)
        elif i % 3 == 1:
            cv2.imwrite(f'./SFR-03(Abnormal)/Albumentation_dataset/LOW_{(i//3)+87}.jpg', albumentation_image)
        else:
            cv2.imwrite(f'./SFR-03(Abnormal)/Albumentation_dataset/NORMAL_{(i//3)+87}.jpg', albumentation_image)
